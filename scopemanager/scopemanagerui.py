...
class ScopeManagerUI(Frame):
    ...
    def createWidgets(self):
        ...
        self.syncButton = Button (self, text='Sync', command = self.sync)
        ...
        self.syncButton.grid(column=5,row=3)
    ...
    #filler functions for now sync aND unsync
    def sync(self, event=None):
        gotopos = radec.RADec.fromStr(self.gotora.get(),self.gotodec.get())
        self.master.messages.log('SYNC '+str(gotopos.ra())+' '+str(gotopos.dec()))
        if self.master.scope is not None and self.master.scope.ready:
            self.master.scope.sync(gotopos)
